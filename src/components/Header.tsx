...
const renderLinks = (links: Array<TopSite | Bookmark>) =>
  links.filter(({ url, faviconUrl }) => url && faviconUrl).slice(0, 5).map((link) => {
...
const LinkSection: React.FC<{ links: Array<TopSite | Bookmark>, marginTop: boolean }> = ({ links, marginTop }) => (
